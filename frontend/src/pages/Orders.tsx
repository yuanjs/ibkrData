      ...
      <div className="flex gap-2 mb-4">
        {(['orders', 'trades', 'pnl'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-1.5 text-sm rounded ${tab === t ? 'bg-blue-600' : 'bg-gray-700'}`}>
            {t === 'orders' ? '订单' : t === 'trades' ? '成交' : '盈亏报告'}
          </button>
        ))}
        {tab === 'trades' && (
          <a href="/api/trades/export" className="ml-auto px-3 py-1.5 text-sm bg-gray-700 rounded hover:bg-gray-600">
            导出CSV
          </a>
        )}
      </div>
          ...
          <table className="w-full text-sm min-w-[500px] md:min-w-0">
            ...
            <tbody>{(orders as Record<string, unknown>[]).map((o, i) => (
              <tr key={i} className="border-b border-gray-800">
                <td className="py-2 px-3 font-mono">{o.symbol as string}</td>
                <td className={`py-2 px-3 ${o.action === 'BUY' ? 'text-green-400' : 'text-red-400'}`}>{o.action as string}</td>
                <td className="py-2 px-3 text-right font-mono">{o.quantity as number}</td>
                <td className="py-2 px-3 text-right font-mono">{o.limit_price as number ?? '-'}</td>
                <td className="py-2 px-3 text-gray-400">{o.status as string}</td>
              </tr>
            ))}</tbody>
          </table>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[600px] md:min-w-0">
            ...
            <tbody>{(trades as Record<string, unknown>[]).map((t, i) => (
              <tr key={i} className="border-b border-gray-800">
                <td className="py-2 px-3 text-gray-400 text-xs">{new Date(t.time as string).toLocaleString()}</td>
                <td className="py-2 px-3 font-mono">{t.symbol as string}</td>
                <td className={`py-2 px-3 ${t.side === 'BOT' ? 'text-green-400' : 'text-red-400'}`}>{t.side as string}</td>
                <td className="py-2 px-3 text-right font-mono">{t.quantity as number}</td>
                <td className="py-2 px-3 text-right font-mono">{t.price as number}</td>
                <td className="py-2 px-3 text-right text-gray-400">{t.commission as number}</td>
              </tr>
            ))}</tbody>
          </table>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[400px] md:min-w-0">
            ...
            <tbody>{(pnl as Record<string, unknown>[]).map((p, i) => (
              <tr key={i} className="border-b border-gray-800">
                <td className="py-2 px-3 font-mono">{p.symbol as string}</td>
                ...
                <td className="py-2 px-3 text-right text-gray-400">{p.trade_count as number}</td>
              </tr>
            ))}</tbody>
          </table>
        </div>
