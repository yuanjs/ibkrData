...
export function History() {
  ...
  const search = async () => {
    if (!symbol) return
    const data = await api.get(`/history/${symbol}?start=${start}&end=${end}&interval=${interval}`)
    setCandles(data)
  }
    ...
    <div className="p-4 space-y-4">
      <div className="flex flex-wrap gap-2 items-end">
        <input value={symbol} onChange={e => setSymbol(e.target.value.toUpperCase())}
          ...
          className="bg-gray-800 border border-gray-600 rounded px-3 py-1.5 text-sm w-32" />
        ...
        <div className="flex gap-1">
          {[['今日', 1], ['近7天', 7], ['近30天', 30]].map(([label, days]) => (
            <button key={label as string} onClick={() => quick(days as number)}
              className="px-2 py-1.5 text-xs bg-gray-700 rounded hover:bg-gray-600">{label}</button>
          ))}
        </div>
        <button onClick={search} className="px-4 py-1.5 text-sm bg-blue-600 rounded hover:bg-blue-500">查询</button>
        {candles.length > 0 && (
          <a href={`/api/history/${symbol}/export?start=${start}&end=${end}&interval=${interval}`}
            className="px-3 py-1.5 text-sm bg-gray-700 rounded hover:bg-gray-600">导出CSV</a>
        )}
      </div>
      {candles.length > 0 && (
