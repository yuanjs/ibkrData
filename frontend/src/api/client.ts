...
export interface FuturesRollState {
  ...
  previous_roll?: Record<string, unknown> | null
  next_roll?: Record<string, unknown> | null
  roll_event_id?: number | null
  effective_from?: string | null
  time?: string
}
...
export const futuresApi = {
  ...
  rollState: (symbol: string) =>
    api.get<FuturesRollState>(`/futures/${symbol}/roll-state`),
  daily: (
    symbol: string,
    start: string,
    asOf?: string,
    adjustment = 'back_adjusted',
    includeLivePartial = false,
    limit?: number,
  ) =>
  ...
  minute: (
    ...
    mode: 'active_raw' | 'adjusted' = 'active_raw',
    asOf?: string,
