...
export function CandleChart({ symbol, data, liveTick, interval, onIntervalChange }: Props) {
  ...
  useEffect(() => {
    ...
    if (!isLineChart && kdjContainerRef.current) {
      ...
      kSeriesRef.current.createPriceLine({ price: 0, color: '#00ff00', lineWidth: 1, lineStyle: 0, axisLabelVisible: true, title: '' })
      kSeriesRef.current.createPriceLine({ price: 100, color: '#00ff00', lineWidth: 1, lineStyle: 0, axisLabelVisible: true, title: '' })
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-2 overflow-x-auto no-scrollbar pb-1">
        {['1w', '1d', '4h', '1h', '15m', '5m', '3m', '2m', '1m', '10s', '5s', '1s'].map((i) => (
          <button
            key={i}
