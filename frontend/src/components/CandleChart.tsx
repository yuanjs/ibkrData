...
export function CandleChart({ symbol, data, liveTick, interval, onIntervalChange }: Props) {
  ...
  useEffect(() => {
    ...
    if (!isLineChart && kdjContainerRef.current) {
      ...
      kSeriesRef.current.createPriceLine({ price: 0, color: '#2d5a27', lineWidth: 1, lineStyle: 0, axisLabelVisible: true, title: '' })
      kSeriesRef.current.createPriceLine({ price: 100, color: '#2d5a27', lineWidth: 1, lineStyle: 0, axisLabelVisible: true, title: '' })
