...
interface MarketStore {
  ...
  getSubscription: (sym: string | null | undefined) => SymbolSubscription | undefined
...
export const useMarketStore = create<MarketStore>((set, get) => ({
  ...
  isFuturesSymbol: (sym) => {
    if (!sym) return false
    return get().subscriptions[sym]?.sec_type === 'FUT'
  },
  getSubscription: (sym) => sym ? get().subscriptions[sym] : undefined,
